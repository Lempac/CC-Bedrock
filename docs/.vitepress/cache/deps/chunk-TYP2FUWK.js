  ...
  if (type === "clear") {
    ...
    if (targetIsArray && key === "length") {
      ...
      depsMap.forEach((dep, key2) => {
        if (
          key2 === "length" || key2 === ARRAY_ITERATE_KEY ||
...
var RefImpl = class {
  ...
  set value(newValue) {
    ...
    const useDirectValue = this["__v_isShallow"] || isShallow(newValue) ||
      isReadonly(newValue);
...
function setRef(rawRef, oldRawRef, parentSuspense, vnode, isUnmount = false) {
  ...
  if (isAsyncWrapper(vnode) && !isUnmount) {
    if (
      vnode.shapeFlag & 512 && vnode.type.__asyncResolved &&
...
function propHasMismatch(el, key, clientValue, vnode, instance) {
  ...
  if (mismatchType != null && !isMismatchAllowed(el, mismatchType)) {
    ...
    const preSegment = `Hydration ${
      MismatchTypeString[mismatchType]
    } mismatch on`;
    const postSegment = `
...
function emit(instance, event, ...rawArgs) {
  ...
  const modifiers = isModelListener2 &&
...
function shouldSetAsProp(el, key, value, isSVG) {
  ...
  if (
    key === "spellcheck" || key === "draggable" || key === "translate" ||
    key === "autocorrect"
  ) {
    return false;
  }
...
var VueElement = class _VueElement extends BaseClass {
  ...
  _createVNode() {
    const baseProps = {};
    if (!this.shadowRoot) {
      baseProps.onVnodeMounted = baseProps.onVnodeUpdated = this._renderSlots
        .bind(this);
    }
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-TYP2FUWK.js.map

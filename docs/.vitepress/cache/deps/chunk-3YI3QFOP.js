...
var injectLocal = (...args) => {
  ...
  if (
    instance && localProvidedStateMap.has(instance) &&
...
function useArrayDifference(...args) {
  ...
  if (symmetric) {
    const diff2 = computed(() =>
      toValue(values).filter((x) =>
        toValue(list).findIndex((y) => compareFn(x, y)) === -1
      )
    );
    return computed(() =>
...
function onClickOutside(target, handler, options = {}) {
  ...
  const listener = (event) => {
    ...
    if (
      !(el instanceof Element) && hasMultipleRoots(target) &&
  ...
  const stopWatch = watchEffect(() => {
    ...
    if (el) {
      const { stop } = useMutationObserver(
        document2,
        (mutationsList) => {
          const targetRemoved = mutationsList.map((
            mutation,
          ) => [...mutation.removedNodes]).flat().some((node) =>
...
function onLongPress(target, handler, options) {
  ...
  function onRelease(ev) {
    ...
    clear();
    if (
      !(options == null ? void 0 : options.onMouseUp) || !_posStart ||
      !_startTimestamp
    ) {
      return;
    }
...
function useBase64(target, options) {
  ...
  function execute() {
    ...
    promise.value.then((res) => {
      base64.value = (options == null ? void 0 : options.dataUrl) === false
...
function useBreakpoints(breakpoints, options = {}) {
  ...
  function current() {
    const points = Object.keys(breakpoints).map((
      k,
    ) => [k, shortcutMethods[k], pxValue(getValue2(k))]).sort((a, b) =>
      a[2] - b[2]
    );
...
function useColorMode(options = {}) {
  ...
  const updateHTMLAttrs = getSSRHandler(
    ...
    (selector2, attribute2, value) => {
      ...
      if (
        classesToAdd.size === 0 && classesToRemove.size === 0 &&
...
function useCountdown(initialCountdown, options) {
  ...
  const intervalController = useIntervalFn(
    ...
    {
      immediate: (_b = options == null ? void 0 : options.immediate) != null
        ? _b
        : false,
    },
  );
  const reset = (countdown) => {
    var _a2;
    remaining.value = (_a2 = toValue(countdown)) != null
      ? _a2
...
function useCssVar(prop, target, options = {}) {
  const { window: window2 = defaultWindow, initialValue, observe = false } =
    options;
      ...
      const onDeviceMotion = createFilterWrapper(
        ...
        (event) => {
          ...
          accelerationIncludingGravity.value = {
            x: ((_d = event.accelerationIncludingGravity) == null
              ? void 0
              : _d.x) || null,
            y: ((_e = event.accelerationIncludingGravity) == null
              ? void 0
              : _e.y) || null,
            z: ((_f = event.accelerationIncludingGravity) == null
              ? void 0
              : _f.z) || null,
          };
          rotationRate.value = {
            alpha: ((_g = event.rotationRate) == null ? void 0 : _g.alpha) ||
              null,
            beta: ((_h = event.rotationRate) == null ? void 0 : _h.beta) ||
              null,
            gamma: ((_i = event.rotationRate) == null ? void 0 : _i.gamma) ||
              null,
          };
...
function useDevicesList(options = {}) {
  ...
  const isSupported = useSupported(() =>
    navigator2 && navigator2.mediaDevices &&
...
function useDropZone(target, options = {}) {
  ...
  if (isClient) {
    ...
    const checkValidity = (items) => {
      const types = Array.from(items != null ? items : []).map((item) =>
        item.type
      );
    ...
    const handleDragEvent = (event, eventType) => {
      ...
      const dataTransferItemList = (_a2 = event.dataTransfer) == null
        ? void 0
        : _a2.items;
      isValid =
    ...
    useEventListener(
      target,
      "dragenter",
      (event) => handleDragEvent(event, "enter"),
    );
    useEventListener(
      target,
      "dragover",
      (event) => handleDragEvent(event, "over"),
    );
    useEventListener(
      target,
      "dragleave",
      (event) => handleDragEvent(event, "leave"),
    );
...
function useFocus(target, options = {}) {
  const { initialValue = false, focusVisible = false, preventScroll = false } =
    options;
...
function useFocusWithin(target, options = {}) {
  ...
  useEventListener(targetElement, EVENT_FOCUS_OUT, () => {
    ...
    return _focused.value =
      (_c = (_b = (_a = targetElement.value) == null ? void 0 : _a.matches) ==
            null
          ? void 0
...
function useSwipe(target, options = {}) {
  ...
  const stops = [
    ...
    useEventListener(target, "touchmove", (e) => {
      ...
      updateCoordsEnd(x, y);
      if (
        listenerOptions.capture && !listenerOptions.passive &&
...
function useTextareaAutosize(options = {}) {
  ...
  const styleProp = (_b = options == null ? void 0 : options.styleProp) != null
    ? _b
    : "height";
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  function constructQuery(params) {
    ...
    const index = hash.indexOf("?");
    if (index > 0) {
      return `${window2.location.search || ""}${hash.slice(0, index)}${
        stringified ? `?${stringified}` : ""
      }`;
    }
    return `${window2.location.search || ""}${hash}${
      stringified ? `?${stringified}` : ""
    }`;
  }
  function read() {
...
function useWebSocket(url, options = {}) {
  ...
  const _init = () => {
    ...
    ws.onclose = (ev) => {
      ...
      if (
        !explicitlyClosed && options.autoReconnect &&
...
function depsParser(deps, localDeps) {
  ...
  const depsFunctionString = localDeps.filter((dep) =>
    typeof dep === "function"
  ).map((fn) => {
...
//# sourceMappingURL=chunk-3YI3QFOP.js.map

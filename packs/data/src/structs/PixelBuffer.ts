...
export const PixelBuffer = new StructType("PixelBuffer", [], [
  new NativeFunction("New", async (interpreter, ctx, start, end, args) => {
    ...
    instance.selfCtx.setProtected(
      "pixelBuffer",
      new Array(width * height).fill(-1),
    );
    return [instance, ctx];
  }),
  ...
  new NativeFunction(
    ...
    async (interpreter, ctx, start, end, args) => {
      ...
      const screenBuffer = selfRef.selfCtx.getProtected<number[]>(
        "pixelBuffer",
      );

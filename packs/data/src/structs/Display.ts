...
const generateLargeDisplayCommands = (
  pixelBuffer: number[],
  origin: Vector3,
  screenWidth: number,
  screenHeight: number,
  ...
  for (var screenXOffset = 0; screenXOffset < screenWidth; screenXOffset++) {
    for (var screenYOffset = 0; screenYOffset < screenHeight; screenYOffset++) {
      ...
      commands.push(
        ...generateDisplayCommands(screenBuffer, screenXOffset, screenYOffset),
      );
    }
  }
...
const generateDisplayCommands = (
  ...
  for (var x = 0; x < 16; x++) {
    for (var y = 0; y < 16; y++) {
      ...
      while (
        y + expandY + 1 < 16 &&
        pixelBuffer[getIdx(x, expandY + 1)] === pixelBuffer[idx]
      ) expandY++;
...
export const Display: StructType = new StructType("Display", [], [
  ...
  new NativeFunction(
    "DrawBuffer",
    async (interpreter, ctx, start, end, args) => {
      ...
      const position = selfRef.selfCtx.getProtected<[number, number, number]>(
        "screenPosition",
      );
      const [width, height] = selfRef.selfCtx.getProtected<[number, number]>(
        "screenSize",
      );
      const blockLocation = { x: position[0], y: position[1], z: position[2] };

      if (width * 16 != bufferWidth || height * 16 != bufferHeight) {
        throw interpreter.runtimeErrorCode(
          `Incorrect size for buffer expected (${width * 16} x ${
            height * 16
          }), instead got (${bufferWidth} x ${bufferHeight})`,
          start,
          end,
        );
      }
      ...
      for (var screenX = 0; screenX < width; screenX++) {
        for (var screenY = 0; screenY < height; screenY++) {
          const screenLoc = {
            x: blockLocation.x + screenX,
            y: blockLocation.y + screenY,
            z: blockLocation.z,
          };
      ...
      const pixels = generateLargeDisplayCommands(
        pixelBuffer,
        blockLocation,
        width,
        height,
      ...
      for (var i = 0; i < pixels.length; i++) {
        ...
        const pixelEntity = overworld.spawnEntity(
          `coslang:pixel<coslang:set_${pixel.x}_${pixel.y}>`,
          blockCorner,
        );
        ...
        if (pixel.width != 0 && pixel.height != 0) {
          pixelEntity.triggerEvent(
            `coslang:size_${pixel.width}_${pixel.height}`,
          );
        }

...
export const FileSystem = {
  directoryTree: (directory: Directory, depth: number = 0): string => {
    ...
    for (let i = 0; i < directory.directories.length; i++) {
      ...
      const pipe =
        (i == directory.directories.length - 1 && directory.files.length == 0)
          ? "└"
          : "├";
      text += `\n${" │ ".repeat(depth)} ${pipe} ${
        FileSystem.directoryTree(dir, depth + 1)
      }`;
    }
  ...
  resolveDirectory: (
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (newPath === undefined) {
        return {
          type: "FileError",
          error: `Directory §3${current.name}§r does not contain §3${
            path[i]
          }§r`,
        };
      }
  ...
  makeDirectory: (
    ...
    if (path.length > 0) {
      ...
      const result = FileSystem.makeDirectory(
    ...
    if (dir.directories.find((f) => f.name == dirName)) {
      return { type: "FileError", error: `${dirName} already exists!` };
    }
  ...
  deleteDirectory: (
    ...
    if (path.length > 0) {
      const index = dir.directories.findIndex((f) => f.name == path.shift());
      const result = FileSystem.deleteDirectory(
        dir.directories[index],
        path,
        dirName,
      );
  ...
  writeFile: (
    ...
    if (fileIdx == -1) {
      return { type: "FileError", error: `${fileIdx} does not exist!` };
    }
  ...
  readFile: (
    ...
    if (file == undefined) {
      return { type: "FileError", error: `${fileName} does not exist` };
    }
    return file;
  },
  ...
  deleteFile: (
    ...
    if (dirIndex == -1) {
      return { type: "FileError", error: `file ${fileName} does not exist!` };
    }
  ...
  makeFile: (
    ...
    if (path.length > 0) {
      const index = dir.directories.findIndex((f) => f.name == path.shift());
      const result = FileSystem.makeFile(
        dir.directories[index],
        path,
        fileName,
      );
    ...
    if (dir.files.find((f) => f.name == fileName)) {
      return { type: "FileError", error: `${fileName} already exists!` };
    }

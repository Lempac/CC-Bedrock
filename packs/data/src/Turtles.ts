...
export const OnTurtlePlace = (e: PlayerPlaceBlockAfterEvent) => {
  ...
  const controller = e.dimension.spawnEntity(
    "coslang:turtle_controller",
    e.block.location,
  );
...
export const OnTurtleInteract = (e: ItemStartUseOnAfterEvent) => {
  ...
  if (turtles.length < 1) {
    return console.log("Unable to find turtle controller!");
  }
  if (turtles.length > 1) {
    return console.log("Multiple turtle controllers found!");
  }

...
const logError = (
  ...
  world.sendMessage(`\n${lineNum} | ` + line);
  world.sendMessage(
    `${" ".repeat(lineNum.toString().length + 3)}` + "§4" +
      `${" ".repeat(startIdx - lineStart)}${"^".repeat(endIdx - startIdx)}`,
  );
...
export class TurtleInterpreter {
  ...
  runScript = async (input: string) => {
    ...
    globals.setSymbol(
      "log",
      new NativeFunction("log", async (interpreter, ctx, start, end, args) => {
    ...
    try {
      ...
      if (interpreter.errMessage === "") {
        world.sendMessage(`${e}, ${e.stack}`);
          ...
          input,
          interpreter.errMessage,
          interpreter.errStart,
          interpreter.errEnd,
        );}
    }
  };
